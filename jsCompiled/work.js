...
const onlyPositive = numbers2.filter((num) => num > 0);
function returnArrOfObj(array) {
    const result = array.map((int) => {
        return { value: int };
    });
    return result;
}
...
function returnS2() {
    const result = items.map(({ title, price }) => ({
        name: title,
        price: price,
    }));
    return result;
...
function returnS3() {
    const result = items.map((obj) => obj.price);
    return result.reduce((a, b) => a + b) / result.length;
}
function returnS4() {
    const result = items.map((obj) => obj.category);
    return result;
}
function returnS5() {
    const result = items.map((obj) => obj.id);
